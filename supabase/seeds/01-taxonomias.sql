-- Seed das taxonomias (doc 04). **É o único seed que roda em produção.**
--
-- Rodado por `supabase db reset` em desenvolvimento e no CI, e aplicado à mão
-- no banco de produção uma vez (docs/runbooks.md). Aqui não há conta, vaga nem
-- evento: só as listas que a área pública oferece como filtro e que a IA
-- consulta para não inventar taxonomia.
--
-- Os slugs são contrato: aparecem na URL dos filtros e são o alvo do
-- mapeamento da IA, então mudá-los quebra links e importações antigas.
--
